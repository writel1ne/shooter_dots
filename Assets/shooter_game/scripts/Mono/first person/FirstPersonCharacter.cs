...
namespace shooter_game.scripts
{
    ...
    public class FirstPersonCharacter : Character
    {
        ...
        [SerializeField]
        private float
            _airStrafeAcceleration =
                100.0f; // Значение из CS:GO для sv_airaccelerate 12, но юниты другие. Нужно подбирать.
        ...
        [Tooltip(
            "The maximum speed the player can reach horizontally in the air using air strafing, before friction/drag caps it. " +
            "Note: Total speed can be higher due to initial jump speed. This primarily limits speed gain from pure strafing.")]
        [SerializeField]
        private float _maxAirSpeed = 30.0f; // В Source это обычно около 30 юнитов/с (sv_airspeed_cap).
        ...
        [SerializeField]
        private float _maxOverallSpeed = 30.0f; // Например, 3500 юнитов/с в Source ~ 60-70 м/с. Подбирать!
        // Если хотите неограниченный банихоп, поставьте очень большое значение.

        [Header("Source Movement Settings")] [SerializeField]
        private float _crouchJumpBonus = 1.0f; // Дополнительный импульс

        [Header("Source Air Movement Specifics")]
        [Tooltip(
            "This value is similar to sv_airaccelerate in Source games. It dictates how much control you have and how quickly you can gain speed by strafing. Needs careful tuning with Max Walk Speed.")]
        [SerializeField]
        private float
            _sourceAirAccelerate =
                100.0f; // Значение по умолчанию, сильно зависит от ваших MaxWalkSpeed и общей настройки.
        // Может быть равно _maxAcceleration или быть отдельной настройкой.
        // В Source (HL2) sv_airaccelerate = 10.

        [Tooltip(
            "This value is similar to cl_airspeed in Quake / sv_maxairspeed in some Source mods. It's a cap on how much speed can be directly 'added' by a strafe input along the wish_dir per tick. Typically set to something like 30 in Source units.")]
        [SerializeField]
        private float _sourceMaxAirWishSpeed = 3.0f; // Если MaxWalkSpeed = 7-10, то это значение может быть 2-4.
        ...
        public FirstPersonCharacterLookInput lookInputComponent { get; private set; }
        public FirstPersonCharacterInput moveInputComponent { get; private set; }
        ...
        protected override void Start()
        {
            ...
            if (TryGetComponent(out FirstPersonCamera _fpv)) firstPersonCamera = _fpv;
        ...
        protected void UpdateRotation(float deltaTime)
        {
            ...
            {
                var fromPlanet = GetPosition() - _planetTransform.position;
                SetGravityVector(fromPlanet.normalized * Math.Clamp(GetGravityMagnitude(), -9.8f, 9.8f));
        ...
        public void AddControlYawInput(float value)
        {
            ...
            {
                AddYawInput(value);
                firstPersonCamera?.AddYawRotation(value);
            }
        }
        ...
        protected override void FallingMovementMode(float deltaTime)
        {
            ...
            var lateralVelocity = Vector3.ProjectOnPlane(characterMovement.velocity, worldUp);
            ...
            var wishDir = GetMovementDirection();
            ...
            if (wishDir.sqrMagnitude > 0.001f)
            {
                ...
                var wishSpeed = _sourceMaxAirWishSpeed;

                var currentSpeedProjection = Vector3.Dot(lateralVelocity, wishDir);
                var addSpeed = wishSpeed - currentSpeedProjection;
                ...
                if (addSpeed > 0) // Только если нам нужно добавить скорость в этом направлении
                {
                    ...
                    var actualAcceleration = _sourceAirAccelerate; // Используем наше новое поле
                    // Если используете GetMaxAcceleration(), он учитывает _airControl.
                    // float actualAcceleration = GetMaxAcceleration(); 

                    var accelAmount = actualAcceleration * deltaTime;
            ...
            else
            {
                ...
                if (fallingLateralFriction > 0.001f && lateralVelocity.sqrMagnitude > 0.001f)
                {
                    ...
                    var drop = speed * fallingLateralFriction * deltaTime;
            ...
            verticalVelocity +=
                gravity * deltaTime; // `gravity` здесь это свойство, которое возвращает GetGravityVector()
            ...
            var gravityDir = GetGravityDirection().normalized; // Направление "вниз"
            var verticalSpeedAlongGravity = Vector3.Dot(verticalVelocity, gravityDir);
            ...
            characterMovement.velocity =
                (lateralVelocity.magnitude <= _sourceMaxAirVelocity
                    ? lateralVelocity
                    : lateralVelocity.normalized * _sourceMaxAirVelocity) + verticalVelocity;
        ...
        protected override bool DoJump()
        {
            ...
            var currentVerticalSpeed = Vector3.Dot(characterMovement.velocity, worldUp);
            var finalVerticalSpeed = jumpImpulse; // Для прыжка с земли, просто берем jumpImpulse
