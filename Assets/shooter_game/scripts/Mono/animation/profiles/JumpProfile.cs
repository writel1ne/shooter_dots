...
namespace shooter_game.scripts.animation.profiles
{
    ...
    public class JumpProfile : BaseCurveProfile<PlayerJumpInputData>
    {
        ...
        protected override void Operation(float deltaTime)
        {
            ...
            {
                animationValue = Mathf.LerpUnclamped(animationValue, inputData.instantValueSeconds > 0 ? _maxValue : _maxCalmValue, inCurve.Evaluate(animationValue) * deltaTime * inMultiplier);
            }
            else
            {
                animationValue = Mathf.LerpUnclamped(animationValue, inputData.instantValueSeconds > 0 ? _maxValue : _minValue, outCurve.Evaluate(animationValue) * deltaTime * outMultiplier);
            }
