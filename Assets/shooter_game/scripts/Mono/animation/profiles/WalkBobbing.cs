...
namespace shooter_game.scripts.animation.profiles
{
    ...
    public class WalkBobbing : BaseCurveProfile<PlayerInputData>
    {
        ...
        protected override void Operation(float deltaTime)
        {
            ...
            {
                animationValue = Mathf.Lerp(animationValue, inputData.vector.magnitude/_maxPlayerSpeed, inCurve.Evaluate(animationValue) * deltaTime * inMultiplier);
            }
            else
            {
                animationValue = Mathf.Lerp(animationValue, 0, outCurve.Evaluate(animationValue) * deltaTime * outMultiplier);
            }
        ...
        {
            float newMagnitude = inputData.vector.magnitude;
            float lastMagnitude = inputData.lastVector.magnitude;
