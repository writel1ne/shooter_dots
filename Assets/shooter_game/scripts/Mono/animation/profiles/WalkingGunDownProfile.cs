...
namespace shooter_game.scripts.animation.profiles
{
    ...
    public class WalkingGunDownProfile : BaseCurveProfile<PlayerInputData>
    {
        ...
        protected override void Operation(float deltaTime)
        {
            ...
            {
                animationValue = Mathf.LerpUnclamped(animationValue, Math.Clamp(inputData.vector.magnitude, _minValue, _maxValue), inCurve.Evaluate(animationValue) * deltaTime * inMultiplier);
            }
            else
            {
